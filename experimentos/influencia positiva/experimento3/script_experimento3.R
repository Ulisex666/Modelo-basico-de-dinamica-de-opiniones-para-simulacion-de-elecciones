...
setwd("C:/Users/ulise/Documents/GitHub/Modelo-base-dinamica-de-opinion/experimentos/influencia positiva/experimento3")
library(tidyverse)

# Se lee la tabla obtenida del experimento y se cambian los nombres de las variables
experimento3_table <- read.csv('experimento3-positiva-table.csv', skip = 6)
...
experimento3_table <- experimento3_table[c('run_number', 'tick', 'agents_per_tick', 'learning_rate' ,'pref_A', 'pref_B')]
...
ggplot(error_experimento3, aes(x = learning_rate, y = agents_per_tick, fill = RMSE)) +
  geom_tile() +
...
error_filtrado <- error_experimento3 %>%
