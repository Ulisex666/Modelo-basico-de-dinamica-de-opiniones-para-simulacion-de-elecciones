...
experimento2_table <- read.csv('secundarios experimento-1-positiva-table.csv', skip = 6)
...
experimento2_table <- experimento2_table[c('run_number', 'tick', 'learning_rate' ,'pref_A', 'pref_B')]
...
ggplot(error_experimento2, aes(x=learning_rate, y = RMSE)) +
...
ggplot(experimento2_mejor_avg, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference share for A with learning-rate = 0.56",
    x = "Ticks (days)",
