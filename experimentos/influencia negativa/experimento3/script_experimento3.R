...
setwd("C:/Users/ulise/Documents/GitHub/Modelo-base-dinamica-de-opinion/experimentos/influencia negativa/experimento3")
library(tidyverse)
...
# Para este experimento se evalúa el efecto del límite de confianza
# en el modelo de influencia negativa. Se eligen las variables de interes
experimento3_table <- experimento3_table[c('run_number', 'tick', 'confidence_threshold' ,'pref_A', 'pref_B')]
...
error_experimento3 <- experimento3_avg %>%
...
head(error_experimento3)

##### Visualización de los resultados ####

ggplot(error_experimento3, aes(x=confidence_threshold, y = RMSE)) +
...
experimento3_mejor <- experimento3_table %>%
  filter(confidence_threshold==1)

experimento3_mejor_avg <- experimento3_mejor %>%
  group_by(tick) %>%
...
ggplot(experimento3_mejor_avg, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Evolución de preferencia por A con learning rate = 0.42,
    8 interacciones por dia",
    x = "Ticks (Días)",
