...
%	\begin{itemize}
%		\item The model was able to approximate the main pattern:opinion share over three time steps and the election results. However, following the POM methodology, we need to evaluate it's capacity to reproduce secondary patterns.
...
%		\item The model was adapted, adding variables for age and gender to each patch. These are read from the first poll and used to initialize the agent's opinion and variables.
%		\item The preference share by gender for option A was calculated at the time of the third poll. It was found that the value was $60\%$ for both genders.
%	\end{itemize}
%\end{frame}
%
%\begin{frame}{Re-evaluating the positive influence model}
%	\begin{itemize}
%		\item Given that it gave the best results, the positive influence model was used to evaluate secondary patterns.
%		\item The optimal parameters were re-calculated given the changes made to the model. The new optimal values are \textit{learning-rate = 0.56} and \textit{agents-updated-per-tick = 7}, with a RMSE of 4.8\%. Almost double compared to the original models, it still has a good approximation for the values found in the third poll.
%	\end{itemize}
%\end{frame}
