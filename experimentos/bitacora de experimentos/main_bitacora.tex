...
\subsubsection{Resultados}
Se toma el promedio de las 30 repeticiones para cada combinación de parámetros como referencia para medir el error comparado con las observaciones reales. Este error se visualiza en la figura \ref{fig:bcexp1error}, con los mejores resultados listados en el cuadro \ref{label}.
...
Como en el resto de modelos, se observa un mejor desempeño con valores alrededor de 0.5 para este parámetro. Al igual que en el resto de modelos, se observa que no se logra reproducir el resultado final del voto, obteniendo una preferencia por la opción A menor que aquella observada en la realidad. Esto indica que para este modelo será necesario aumentar el número de agentes interactuando por día, lo cuál conlleva cambios al parámetro de \textit{agents per tick} y \textit{confidence threshold}. Se inicia evaluando cambios para el primero, manteniendo fijo el segundo. 
